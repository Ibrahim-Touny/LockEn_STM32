/**
 * @file    ESP8266.c
 * @brief   ESP8266 WiFi module AT command interface for network configuration,
 *          AP mode setup, and TCP/UDP communication.
 */

#include "Drivers/ESP8266.h"
#include "Drivers/ESP8266Config.h"

Wifi_t Wifi;

/**
 * @brief Send raw data bytes to the ESP8266 via UART.
 *
...
/**
 * @brief Send a null-terminated string to the ESP8266.
 *
 * @param data Pointer to string (e.g., "AT\r\n")
 * @return true on success, false on failure
 */
...
/**
 * @brief Send a string and wait a specified time.
 *
 * @param data Pointer to string
 * @param DelayUs Delay in microseconds after sending
...
/**
 * @brief Wait for one of several expected response strings from the ESP8266.
 *
 * Uses variadic arguments to specify multiple response strings to match against.
 * Returns which response was found first.
 *
 * @param TimeOut_ms Timeout in milliseconds
 * @param result Pointer to store index of matched string (1-based)
 * @param CountOfParameter Number of response strings to wait for
 * @param ... Variable list of response strings (const char*)
 * @return true if a match was found, false on timeout
 */
...
/**
 * @brief Extract a specific field from the last received response.
 *
 * Splits the response buffer by delimiter and returns the N-th field.
 *
 * @param result Output buffer for the extracted field
 * @param WantWhichOne Which field to extract (1-based index)
 * @param SplitterChars Delimiter character(s) for splitting
 * @return true on success, false on failure
 */
...
/**
 * @brief Extract multiple fields from the response buffer.
 *
 * Splits the response and stores multiple fields into output buffers.
 *
 * @param InputString Response buffer to parse
 * @param SplitterChars Delimiter character(s)
 * @param CountOfParameter Number of fields to extract
 * @param ... Variable list of output buffers (char*)
 * @return true on success, false on failure
 */
...
/**
 * @brief Extract a field and convert it to an integer.
 *
 * @param result Output integer pointer
 * @param WantWhichOne Which field to extract (1-based)
 * @param SplitterChars Delimiter character(s)
 * @return true on success, false on failure
 */
...
/**
 * @brief Extract a field and convert it to a float.
 *
 * @param result Output float pointer
 * @param WantWhichOne Which field to extract (1-based)
 * @param SplitterChars Delimiter character(s)
 * @return true on success, false on failure
 */
...
/**
 * @brief Clear the UART receive buffer and start fresh interrupt reception.
 */
...
/**
 * @brief Clear the UART transmit buffer.
 */
...
/**
 * @brief UART receive interrupt callback (called per byte).
 *
 * Accumulates incoming bytes into the receive buffer.
 */
...
/**
 * @brief Initialize the ESP8266 module and verify it responds to AT commands.
 *
 * @return true if module responds, false if not communicating
 */
bool Wifi_Init(void)
{
	uint8_t result;
...
/**
 * @brief Power up the ESP8266 module by toggling enable and reset pins.
 *
 * Waits 2 seconds for the module to fully boot after power-up.
 */
...
/**
 * @brief Power down the ESP8266 module.
 */
