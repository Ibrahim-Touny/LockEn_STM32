/**
 * @file    R307.c
 * @brief   Fingerprint sensor (R307) UART driver for enrollment and verification
...
/**
 * @brief Enable or disable debug logging for fingerprint sensor.
 *
 * @param enable 1 to enable, 0 to disable logging
 */
...
/**
 * @brief Log a debug message to the LCD display if logging is enabled.
 *
...
/**
 * @brief Build an R307 command packet with header, instruction, and checksum.
 *
 * @param out_buf Output buffer (must be > 32 bytes)
 * @param instruction Command instruction byte (0x01-0x13)
 * @param params Parameter array (can be NULL)
 * @param params_len Length of parameters
 * @return Total length of the built packet
 */
...
/**
 * @brief Send an R307 command and receive the response.
 *
 * Transmits the command packet and reads the response from UART.
 * Returns HAL_OK even if R307 returns an error code (check response[9]).
 *
 * @param cmd Command packet to send
 * @param cmd_len Command length
 * @param response Output buffer for response packet
 * @param response_max_len Max response size
 * @return HAL_OK on UART success, HAL_ERROR on UART failure
 */
 ...
 *
 * @return HAL_OK if finger detected, HAL_ERROR if no finger or error
 */
...
/**
 * @brief Convert captured image to characteristic file in specified buffer.
 *
 * @param buffer_id Buffer to use (1 or 2)
...
/**
 * @brief Generate a fingerprint template by merging buffer1 and buffer2.
 *
 * @return HAL_OK on success, HAL_ERROR on failure
 */
...
/**
 * @brief Store a generated template to the sensor's flash memory.
 *
 * @param buffer_id Buffer to store from (typically 1)
 * @param page_id Template ID in flash (0 to max)
 * @return HAL_OK on success, HAL_ERROR on failure
 */
...
/**
 * @brief Search the sensor's template library for a match.
 *
 * @param out_page_id Output matched template ID
 * @param out_score Output match score
 * @param start_page Starting search address
 * @param page_num Number of templates to search
 * @return HAL_OK if match found, HAL_ERROR if no match
 */
...
/**
 * @brief Delete one or more templates from sensor flash.
 *
...
/**
 * @brief Erase all templates from sensor flash memory.
 *
 * @return HAL_OK on success, HAL_ERROR on failure
 */
...
/**
 * @brief Wait for a finger to be lifted from the sensor.
 *
 * @param timeout_ms Maximum wait time in milliseconds
 * @return HAL_OK if finger removed, HAL_TIMEOUT on timeout
 */
...
/**
 * @brief Wait for a finger to be placed on the sensor.
 *
 * @param timeout_ms Maximum wait time in milliseconds
 * @return HAL_OK if finger detected, HAL_TIMEOUT on timeout
 */
...
/**
 * @brief Capture a fingerprint image with timeout handling.
 *
...
/**
 * @brief Complete fingerprint enrollment for a given template ID.
 *
 * Captures finger twice, generates a template, and stores it.
 *
 * @param page_id Template ID to store enrollment under
 * @return HAL_OK on success, HAL_ERROR on failure, HAL_TIMEOUT on user timeout
 */
...
/**
 * @brief Verify a fingerprint against enrolled templates.
 *
 * Captures and searches the template library.
 *
 * @param out_page_id Output matched template ID
 * @param out_score Output match confidence score
 * @return HAL_OK if match found, HAL_ERROR if no match, HAL_TIMEOUT if timeout
 */
...
/**
 * @brief Verify the sensor module password for authentication.
 *
 * Default password is 0x00000000. Must be called before other commands.
 *
 * @param password 32-bit password (default 0x00000000)
 * @return HAL_OK if password is correct, HAL_ERROR if not
 */
...
/**
 * @brief Set a new password on the R307 module.
 *
 * @param new_password New 32-bit password
 * @return HAL_OK on success, HAL_ERROR on failure
 */
...
/**
 * @brief Clear all fingerprint templates (convenience wrapper).
 *
 * @return HAL_OK on success, HAL_ERROR on failure
 */
...
/**
 * @brief Delete a range of fingerprint templates (convenience wrapper).
 *
 * @param from_ID Starting template ID
 * @param num_fp Number of templates to delete
 * @return HAL_OK on success, HAL_ERROR on failure
 */
