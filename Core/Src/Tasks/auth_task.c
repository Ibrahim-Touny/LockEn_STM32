/**
 * @file    auth_task.c
 * @brief   Main authentication task - handles device setup, credential storage,
 *          multi-factor authentication (RFID, fingerprint, password, face),
 *          solenoid unlocking, tamper detection, and session management.
 */
...
/**
 * @brief Build and send an authentication event log entry to EspTask.
 *
 * Constructs a log entry with the event name and a compact string of the
 * factors that triggered the event (RFID, FP, PWD, CAM). Sends it to the
 * ESP log queue for remote upload to the server.
 *
 * @param event Event name (e.g., "GRANTED", "TAMPER", "TIMEOUT")
 * @param fx_mask Bitmask of authentication factors involved
 */
...
/**
 * @brief Unlock the solenoid by setting its GPIO pin high.
 */
static void Solenoid_Unlock(void) { HAL_GPIO_WritePin(SOLENOID_PORT, SOLENOID_PIN, GPIO_PIN_SET);   }

/**
 * @brief Lock the solenoid by setting its GPIO pin low.
 */
static void Solenoid_Lock(void)   { HAL_GPIO_WritePin(SOLENOID_PORT, SOLENOID_PIN, GPIO_PIN_RESET); }

/**
 * @brief Check if the reset button is currently pressed (GPIO reads low when pressed).
 *
...
/**
 * @brief Read a password from the keypad during initial setup.
 *
 * Reads digits from keypad into a buffer, displays '*' for each digit,
 * allows backspace with '*', and confirms entry with '#'.
 * Runs during setup before credentials are active.
 *
 * @param entered Output buffer to store the password string (null-terminated)
 * @param max_len Maximum password length (typically 4)
 */
...
/**
 * @brief Scan and store an RFID card during initial setup.
 *
 * Waits for an admin card, reads its UID, and stores it as the authentication card.
 * Displays the UID hex string after a successful scan.
 */
...
/**
 * @brief Enroll fingerprint(s) during initial setup.
 *
 * Guides the user to place a finger multiple times (NUM_FINGERS = 1).
 * For each finger, attempts enrollment with retries on failure.
 */
...
/**
 * @brief Register a PIN password during initial setup.
 *
 * Uses get_password() to read a 4-digit PIN and stores it in credentials.
 */
...
/**
 * @brief Enroll a face during initial setup.
 *
 * Requests EspTask to begin face capture and waits for enrollment confirmation.
 */
...
/**
 * @brief Initialize all hardware sensors and modules.
 *
 * Powers up RFID, performs I2C bus reset, initializes LCD, checks RFID presence,
 * initializes MPU6050 for tamper detection, and initializes the fingerprint sensor.
 * Displays status messages and alerts if sensors are missing.
 */
...
/**
 * @brief Run initial device setup: card, fingerprint, password, and face enrollment.
 *
 * Erases old credentials, guides user through registering all factors,
 * saves them to flash, and confirms completion.
 */
...
/**
 * @brief Main authentication task - central coordinator for access control.
 *
 * Creates the auth event queue, initializes hardware, loads or enrolls credentials.
 * Maintains a session-based state machine that collects authentication factors
 * from sensor tasks. Once 2+ different factors are received within 30 seconds,
 * unlocks the solenoid. Handles reset button to re-enter setup, tamper events,
 * and idle-based sleep entry.
 *
